...
types are defined in such a way that the validation of values and
structural validation does *not* happen as it does during
deserialization.  For example, the :attr:`colander.null` value is
substituted for every missing subvalue in a serialization value, and
none of the validators associated with the schema or any of is nodes
is invoked.
...
``name`` attribute in the data we fed to ``serialize``, an error is
*not* raised.  For more information about :attr:`colander.null`
